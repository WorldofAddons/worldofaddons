...
export function checkWoAVersion () {
  return new Promise((resolve, reject) => {
    ...
    req.open('GET', 'https://api.github.com/repos/WorldofAddons/worldofaddons/releases/latest', true) // Use github API to check version
    req.send(null)
    req.onreadystatechange = () => {
      if (req.readyState === 4) {
        let allReleases = JSON.parse(req.responseText)
        let latestRelease = allReleases.tag_name.split('-')
